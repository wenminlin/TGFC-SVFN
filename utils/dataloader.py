...
class SVFENDDataset(Dataset):
    ...
    def __init__(self, path_vid, datamode='title+ocr'):
        ...
        self.data_complete = pd.read_json('./data/data_complete.json',orient='records',dtype=False,lines=True)
        self.data_complete = self.data_complete[self.data_complete['annotation']!="辟谣"] # annotation: 0-real, 1-fake, 2-debunk
        ...
        self.tokenizer  = BertTokenizer.from_pretrained(r"/root/autodl-tmp/new_fakesv/FakeSV-main/code/data/bert-base-chinese")

        self.datamode = datamode
        self.data_gpt= pd.read_json(r'/root/autodl-tmp/new_fakesv/FakeSV-main/code/data/gptdata/gpt_time_all_data.json',orient='records',dtype=False,lines=True)
    ...
    def __getitem__(self, idx):
        ...
        # 确保两种数据都是字符类型
        self.data_gpt['新闻id'] = self.data_gpt['新闻id'].astype(str)
        vid = str(vid)
        
        # 加载gpt数据
        gpt_data=self.data_gpt[self.data_gpt['新闻id'] == vid].iloc[0]
        # print(gpt_data)
        
        gpt_data_tokens=self.tokenizer("预测结果：" + gpt_data["预测结果"] +"[SEP]" + "理由和解释：" + gpt_data["理由和解释"], max_length=128, padding='max_length', truncation=True)
        
        gpt_data_inputid = torch.LongTensor(gpt_data_tokens['input_ids'])
        gpt_data_mask = torch.LongTensor(gpt_data_tokens['attention_mask'])

        
        # print("123",item['title'])
        vid = item['video_id']
        # label

        
        if item['annotation'] == '真':
            label = 0
        elif item['annotation'] == '假':
            label = 1
        else:
            label = 2  
        ...
        for key in ['author_intro', 'author_verified_intro']:
            try:
                ...
                intro += '  '
        intro_tokens = self.tokenizer(intro, max_length=128, padding='max_length', truncation=True)
        ...
        return {
            ...
            # 'vid':vid,
            'gpt_data_inputid':gpt_data_inputid,
            'gpt_data_mask':gpt_data_mask,
        }
