...
class Run():
    ...
    def get_model(self):
        if self.model_name == 'SVFEND':
            ...
            self.model = Fusion(bert_model='bert-base-chinese', fea_dim=128,dropout=self.dropout)
    ...
    def main(self):
        if self.mode_eval == "nocv":
            ...
            for fold in range(1, 6): 
                ...
                trainer = TrainerKD_T(model = self.model, device = self.device, lr = self.lr, dataloaders = dataloaders, epoches = self.epoches, dropout = self.dropout, weight_decay = self.weight_decay, mode = self.mode, model_name = self.model_name, event_num = self.event_num, 
